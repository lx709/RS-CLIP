...
def load_sample(sample, labels, label_type, threshold, imgTransform, use_s1, use_s2, use_RGB, IGBP_s):
    ...
    ss = rt_sample["id"].split('_')
    path = os.path.join("_".join(ss[0:2]), "_".join(ss[2:4]), rt_sample["id"])
    
    return rt_sample['image'], loc, path
...
class SEN12MS(data.Dataset):
    ...
    def __init__(self, path, ls_dir=None, imgTransform=None, season=None,
                 label_type="multi_label", threshold=0.1, subset="train",
                 use_s2=False, use_s1=False, use_RGB=False, IGBP_s=True, txt_path='all.txt', ):
...
#         data = pd.read_excel(os.path.join(path, 'IGBP_Landuse.xls'))
#         self.classes = data['Class name'].values.astype('str')
#         if IGBP_s == True:
#             lc = self.classes
#             cls1 = 'Forest'
#             cls2 = 'Shrublands'
#             cls3 = 'Savannas'
#             cls6 = 'Croplands'
#             self.classes = np.asarray([cls1, cls2, cls3, lc[9], lc[10], cls6, lc[12], lc[14], lc[15], lc[16]])
    # save as above
        self.classes = ['Forest', 'Shrublands', 'Savannas', 'Grasslands', 'Permanent wetlands', 'Croplands', 'Urban and builtup lands', 'Snow and ice', 'Barren', 'Water bodies']
    
#         self.classes = ['forest', 'shrubland', 'savanna', 'grassland', 'wetland', 'cropland', 'urban and built-up land', 'snow and ice land', 'barrenland', 'waterbody']
        
#         self.classes = ['forest', 'shrubland', 'savanna', 'grassland', 'wetland', 'permanent cropland', 'urban and builtup land', 'snow and ice', 'barren', 'waterbody']
...
#-------------------------------- import split lists--------------------------------
        if label_type == "multi_label" or label_type == "single_label":
            ...
            if season in ['sprint', 'summer', 'fall', 'winter']:
                sample_list = [dd for dd in sample_list if season in dd] ## added by xiang
            
            # remove broken file
            broken_files = ['ROIs1868_summer_s2_146_p202.tif', 'ROIs1868_summer_s2_85_p273.tif']
...
#----------------------------------------------------------------------               
        ...
        label_path = os.path.dirname(ls_dir)
        label_path = os.path.dirname(label_path)
        label_file = os.path.join(label_path, 'labels', 'IGBP_probability_labels.pkl')
...
class SEN12_Pseudo_Dataset(Dataset): # labels already converted in consistent with train set
    def __init__(self, data_root='./data/SEN12MS/', txt_path='all_summer.txt', transform=None, test=False, novel_class=None):
    ...
    def __getitem__(self, index):
        ...
        im_path = os.path.join(self.data_root, im_path0)
        
        img = load_s2(im_path, self.transform, s2_band=S2_BANDS_RGB)
        
        rt_sample = {'image': img, 'label': label, 'id': im_path0}
        
        if self.transform is not None:
            rt_sample = self.transform(rt_sample)
        
        return rt_sample['image'], label, im_path0
...
#             T.Normalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711))
        ...
        rt_sample = {'image': img, 'label':label, 'id':sample_id}
        return rt_sample
...
if __name__ == "__main__":
    ...
    # data path
    data_dir = "./SEN12MS/"    # SEN12MS dir
    list_dir = "./SEN12MS/SEN12MS/splits/"    # split lists/ label dirs

    # define image transform
    imgTransform=T.Compose([ToTensor(),Normalize(bands_mean, bands_std)])
