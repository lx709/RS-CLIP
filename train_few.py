...
def parse_args(argv):
    ...
    parser.add_argument('--n-shots', default=5, type=int, required=False,
                        help='Number of classes to train with')
    parser.add_argument('--num-workers', default=4, type=int, required=False,
                        help='Number of subprocesses to use for the dataloader (default=%(default)s)')
    parser.add_argument('--pseudo-version', default=2, type=int, required=False,
    ...
    parser.add_argument('--nbatches', default=1000, type=int, required=False,
...
def run_training(args):
    ...
    optimizer = optim.Adam(model.parameters(), lr=args.lr,betas=(0.9,0.98),eps=1e-6,weight_decay=0.2) #Params used from paper, the lr is smaller, more safe for fine tuning to new dataset
    ...
    if 'sen12ms' in args.dataset.lower():
        ...
        train_dataset = RSDataset(data_root=data_root, txt_path='train_{}shot.txt'.format(args.n_shots), transform=train_transforms)
    ...
    while n_batch < args.nbatches:
        ...
        for images, targets, _ in train_loader:
            ...
            texts = [prompt + train_loader.dataset.classes[t] for t in targets]
